...
import {
  ...
  type FindingSection, type FindingStatus, type FindingRow,
} from "../../lib/journey";
...
const FINDING_CATEGORIES: { id: "all" | FindingCategory; label: string }[] = [
...
function SourceChip({ label }: { label: string }) {
  return (
    <span className="inline-flex items-center gap-1 rounded-md border border-mist/70 bg-white px-1.5 py-0.5 text-[10.5px] font-medium text-smoke">
      <FileText className="h-3 w-3 text-electric" /> {label}
    </span>
  );
}
...
// Keep legacy exports referenced elsewhere
function StatusPill({ status }: { status: FindingStatus }) {
  const map: Record<FindingStatus, { label: string; cls: string; dot: string }> = {
  ...
  const s = map[status];
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full border px-2 py-0.5 text-[10.5px] font-semibold ${s.cls}`}>
      <span className={`h-1.5 w-1.5 rounded-full ${s.dot}`} /> {s.label}
    </span>
  );
}

// Preserve reference so tree-shaking doesn't complain about unused imports
void StatusPill; void findingSections;
